...
import { BASE_URL, HEADERS_AUTH, HEADERS_AUTH_FORM } from "../../../utils";
...
export const CreateEvidence = createAsyncThunk(
    ...
    async ({ projectId, file }: any) => {
        ...
        formData.append('UserIn', (localStorage.getItem('userData') as any)?.email);

        const response = await axios.post(`${BASE_URL}Garden/Evidence`, formData, HEADERS_AUTH_FORM);
...
export const ApplyProject = createAsyncThunk(
    ...
    async ({ projectId, file }: any) => {
        ...
        const response = await axios.post(`${BASE_URL}Garden/ApplyProject`, formData, HEADERS_AUTH_FORM);
